...
RUN apk add sudo bash xvfb xdpyinfo x11vnc chromium python3 py3-pip git openssl curl gcc libc-dev python3-dev php socat python3-tkinter && \
...
RUN echo 'RESOLUTION=$(head -1 /tmp/resolution.txt)' > /home/user/startVNC.sh && \
    echo 'IFS=x read -ra arr <<< "$RESOLUTION" && arr[0]=$((arr[0]+1)) && arr[1]=$((arr[1]+1)) && sudo sed -i "s/WIDTH,HEIGHT/${arr[0]},${arr[1]}/" /etc/chromium/chromium.conf' >> /home/user/startVNC.sh && \
